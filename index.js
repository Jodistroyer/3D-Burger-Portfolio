...
window.addEventListener("touchmove", (event) => {
    ...
    if (Math.abs(deltaY) > 40) { // Prevent accidental small movements
        scrollStep = Math.max(0, Math.min(scrollStep + (deltaY > 0 ? 1 : -1), layers.length + 4));
        updateTexts();
        touchStartY = touchEndY; // Reset for smoother scrolling
    }
});
